...
struct my_call_data *Call::call_init_tonegen(){
    pjsua_call_id call_id = this->id;

    PJ_LOG(1, (THIS_FILE, "Panyi call_init_tonegen call_id = %d" , call_id));
    ...
    PJ_LOG(1, (THIS_FILE, "Panyi call_init_tonegen end!"));

    return cd;
}

void Call::call_play_digit_inband(const char *digits){
    pjsua_call_id call_id = this->id;

    PJ_LOG(1, (THIS_FILE, "Panyi call_play_digit_inband call_id = %d" , call_id));

    pjmedia_tone_digit d[16];
    unsigned count = strlen(digits); 

    if(cd == NULL){
        PJ_LOG(1, (THIS_FILE, "Panyi call_init_tonegen in call inband call_id = %d" , call_id));
        cd = call_init_tonegen();
    }
    ...
    //send digit media
    pjmedia_port *port = cd->tonegen;
    PJ_LOG(1, (THIS_FILE, "Panyi pjmedia_tonegen_play_digits cd->tone count=%d" ,count));
    PJ_LOG(1, (THIS_FILE, "Panyi pjmedia_tonegen_play_digits cd->tone port=%ld" ,port));
    int play_send_result = pjmedia_tonegen_play_digits(cd->tonegen , count , d , 0);
    PJ_LOG(1, (THIS_FILE, "Panyi send %s play_send_result = %d" ,digits ,play_send_result));
}

void Call::call_deinit_tonegen(){
    pjsua_call_id call_id = this->id;
    
    PJ_LOG(1, (THIS_FILE, "Panyi call_deinit_tonegen call_id = %d" , call_id));
